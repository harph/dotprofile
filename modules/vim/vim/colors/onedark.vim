...
call s:h("MatchParen", { "bg": s:dark_yellow, "fg": s:dark_blue }) " The character under the cursor or just before it, if it is a paired bracket, and its match.
...
call s:h("StatusLine", { "fg": s:green, "bg": s:vertsplit }) " status line of current window
call s:h("StatusLineNC", { "fg": s:dark_green, "bg": s:vertsplit}) " status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
